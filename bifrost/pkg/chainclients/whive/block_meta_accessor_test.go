...
type WhiveBlockMetaAccessorTestSuite struct{}

var _ = Suite(
	&WhiveBlockMetaAccessorTestSuite{},
)

func (s *WhiveBlockMetaAccessorTestSuite) TestNewBlockMetaAccessor(c *C) {
...
func (s *WhiveBlockMetaAccessorTestSuite) TestBlockMetaAccessor(c *C) {
	memStorage := storage.NewMemStorage()
	db, err := leveldb.Open(memStorage, nil)
	c.Assert(err, IsNil)
