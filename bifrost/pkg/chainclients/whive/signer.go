...
func getWHIVEPrivateKey(key cryptotypes.PrivKey) (*btcec.PrivateKey, error) {
...
func (c *Client) getGasCoin(tx stypes.TxOutItem, vSize int64) common.Coin {
	...
	return common.NewCoin(common.WHIVEAsset, cosmos.NewUint(uint64(gasRate*vSize)))
}
...
func (c *Client) getSourceScript(tx stypes.TxOutItem) ([]byte, error) {
	sourceAddr, err := tx.VaultPubKey.GetAddress(common.WHIVEChain)
	if err != nil {
...
func (c *Client) SignTx(tx stypes.TxOutItem, thorchainHeight int64) ([]byte, error) {
	if !tx.Chain.Equals(common.WHIVEChain) {
		return nil, errors.New("not WHIVE chain")
	}
	...
	txes, err := c.getUtxoToSpend(tx.VaultPubKey, c.getWHIVEPaymentAmount(tx))
	if err != nil {
	...
	coinToCustomer := tx.Coins.GetCoin(common.WHIVEAsset)
	totalSize := c.estimateTxSize(tx.Memo, txes)

	// whived has a default rule max fee rate should less than 0.1 WHIVE / kb
	...
	// maxFee in sats
	maxFeeSats := totalSize * defaultMaxWHIVEFeeRate / 1024
	gasCoin := c.getGasCoin(tx, totalSize)
	gasAmtSats := gasCoin.Amount.Uint64()

	// make sure the transaction fee is not more than 0.1 WHIVE / kb , otherwise it might reject the transaction
	...
	if !tx.MaxGas.IsEmpty() {
		maxGasCoin := tx.MaxGas.ToCoins().GetCoin(common.WHIVEAsset)
...
// BroadcastTx will broadcast the given payload to WHIVE chain
func (c *Client) BroadcastTx(txOut stypes.TxOutItem, payload []byte) (string, error) {
	...
	c.logger.Info().Str("hash", txHash.String()).Msg("broadcast to WHIVE chain successfully")
...
func (c *Client) consolidateUTXOs() {
	...
	for _, vault := range vaults {
		...
		addr, err := vault.PubKey.GetAddress(common.WHIVEChain)
		if err != nil {
			c.logger.Err(err).Msgf("fail to get WHIVE address for pubkey:%s", vault.PubKey)
			continue
		}
		...
		txOutItem := stypes.TxOutItem{
			...
			Coins: common.Coins{
				common.NewCoin(common.WHIVEAsset, cosmos.NewUint(uint64(amt))),
			},
			Memo:    "consolidate",
			MaxGas:  nil,
