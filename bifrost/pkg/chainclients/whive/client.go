...
// Client observes whive chain and allows to sign and broadcast tx
type Client struct {
...
func NewClient(thorKeys *thorclient.Keys, cfg config.ChainConfiguration, server *tssp.TssServer, bridge *thorclient.ThorchainBridge, m *metrics.Metrics) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("fail to create whive rpc client: %w", err)
	}
	...
	whivePrivateKey, err := getWHIVEPrivateKey(thorPrivateKey)
	if err != nil {
		return nil, fmt.Errorf("fail to convert private key for WHIVE: %w", err)
	}
	ksWrapper, err := NewKeySignWrapper(whivePrivateKey, tssKm)
	if err != nil {
	...
	c := &Client{
		logger:           log.Logger.With().Str("module", "whive").Logger(),
		cfg:              cfg,
		chain:            cfg.ChainID,
		client:           client,
		privateKey:       whivePrivateKey,
		ksWrapper:        ksWrapper,
		bridge:           bridge,
...
// GetChain returns WHIVE Chain
func (c *Client) GetChain() common.Chain {
	return common.WHIVEChain
}
...
func (c *Client) GetAddress(poolPubKey common.PubKey) string {
	addr, err := poolPubKey.GetAddress(common.WHIVEChain)
	if err != nil {
...
func (c *Client) getUTXOs(minConfirm, MaximumConfirm int, pkey common.PubKey) ([]btcjson.ListUnspentResult, error) {
	whiveAddress, err := pkey.GetAddress(common.WHIVEChain)
	if err != nil {
...
func (c *Client) GetAccount(pkey common.PubKey) (common.Account, error) {
	...
	return common.NewAccount(0, 0,
		common.Coins{
			common.NewCoin(common.WHIVEAsset, cosmos.NewUint(uint64(totalAmt))),
		}, false), nil
}
...
func (c *Client) getAsgardAddress() ([]common.Address, error) {
	...
	for _, v := range vaults {
		addr, err := v.PubKey.GetAddress(common.WHIVEChain)
		if err != nil {
...
// For whive chain client we want to save the utxo we can spend later to sign
...
// reConfirmTx will be kicked off only when chain client detected a re-org on whive chain
// it will read through all the block meta data from local storage , and go through all the UTXOs.
// For each UTXO , it will send a RPC request to whive chain , double check whether the TX exist or not
...
func (c *Client) FetchTxs(height int64) (types.TxIn, error) {
	...
	if err != nil {
		c.logger.Err(err).Msg("fail to process whive re-org")
	}
	...
	// WHIVE has faster block time, report every 5 blocks seems fine
	if height%5 == 0 {
...
func (c *Client) sendNetworkFee(height int64) error {
	...
	if c.lastFeeRate != feeRate {
		txid, err := c.bridge.PostNetworkFee(height, common.WHIVEChain, uint64(EstimateAverageTxSize), feeRate)
		if err != nil {
...
func (c *Client) getTxIn(tx *btcjson.TxRawResult, height int64) (types.TxInItem, error) {
	...
	return types.TxInItem{
		...
		To:          toAddr,
		Coins: common.Coins{
			common.NewCoin(common.WHIVEAsset, cosmos.NewUint(amt)),
		},
		Memo: memo,
		Gas:  gas,
	}, nil
...
//
// we expect array of "vout" for a WHIVE to have this format
...
// getSender returns sender address for a whive tx, using vin:0
func (c *Client) getSender(tx *btcjson.TxRawResult) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("fail to query raw tx from whived")
	}
...
// getMemo returns memo for a whive tx, using vout OP_RETURN
...
// getGas returns gas for a whive tx (sum vin - sum vout)
func (c *Client) getGas(tx *btcjson.TxRawResult) (common.Gas, error) {
	...
	return common.Gas{
		common.NewCoin(common.WHIVEAsset, cosmos.NewUint(totalGas)),
	}, nil
}
...
func (c *Client) registerAddressInWalletAsWatch(pkey common.PubKey) error {
	addr, err := pkey.GetAddress(common.WHIVEChain)
	if err != nil {
		return fmt.Errorf("fail to get WHIVE address from pubkey(%s): %w", pkey, err)
	}
	c.logger.Info().Msgf("import address: %s", addr.String())
	return c.client.ImportAddressRescan(addr.String(), "", false)
}

// RegisterPublicKey register the given pubkey to whive wallet
...
func (c *Client) getBlockRequiredConfirmation(txIn types.TxIn, height int64) (int64, error) {
	totalTxValue := txIn.GetTotalTransactionValue(common.WHIVEAsset, c.asgardAddresses)
...
func (c *Client) reportSolvency(whiveBlockHeight int64) error {
